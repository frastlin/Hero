...
def lister(l=[]):
	"""Puts the argument into a list"""
	li = []
	map(li.append, l)
	return li

def menu(key_pressed, title="test menu", options=["yes", "no"], keys=['up', 'down'], state=KEYDOWN):
	...
	options_list = lister(options)
	name1 = key_pressed
#		audio.menu_sound(name1)
	if name1 in keys:
		current_value = scroll(current_value, len(options_list), keys, name1)
		spk("%s. %s" % (options_list[current_value], current_value + 1))
	elif name1 in ['`','1','2','3','4','5','6','7','8','9','0','-','=']:
		o = hotkey(name1, options_list)
		if o != "duck":
			spk(options_list[o])
			return ("attack_choice", options[options_list[o]])
	elif name1 == "return":
		return ("attack_choice", options[options_list[current_value]])
